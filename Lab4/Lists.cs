...
namespace Lab4_C_
{
    internal class Lists
    {
        ...
        public static void delIfSameNeighbors(LinkedList<string> l)
        {
            ...
            LinkedListNode<string> now = l.First;
            while (now.Next != null)
            {
                if (now.Previous == null)
                {
                    if (now.Next.Value == l.Last.Value)
                    {
                        l.RemoveFirst();
                        now = l.First;
                    }
                ...
                else
                {
                    ...
                    {
                        LinkedListNode<string> remove = now;
                        now = now.Next;
                        l.Remove(remove);
                    }
                    else
            ...
            if (now.Previous.Value == l.First.Value)
            {
                l.RemoveLast();
            }
        ...
        public static void FillStoresData(string filePath)
        {
            ...
            try
            {
                for (int i = 0; i < n; i++)
                {
                    ...
                    if (firm.Length > 20 || street.Length > 20 || !(fatContent == 15 || fatContent == 20 || fatContent == 25) || price < 1) 
                    {
                        Console.WriteLine("Введённые данные не соответствуют условию, последняя строка будет проигнорированна"); continue;
                    }
