...
namespace Lab5C_
{
    internal class ExcelWork
    {
            ...
            try
            {
                ...
                using (var package = new ExcelPackage(fileInfo))
                {
                    ...
                    for (int row = 2; row <= rowCount; row++)
                    {
                        ...
                        DateTime date = Convert.ToDateTime(worksheet.Cells[row, 3].Text);
        ...
        public static bool deleteClient(List<Client> clients, int key, List<Order> orders)
        {
            int clientCode;
            string input;
            Console.WriteLine("Введите код клиента для удаления (будет произведено каскадное удаление)");
            input = Console.ReadLine();
            if (input != "" && int.TryParse(input, out clientCode))
            {
                if (clients.Any(item => item.code == clientCode))
                { clients.RemoveAll(item => item.code == clientCode); orders.RemoveAll(item => item.clientCode == clientCode); }
                else { Console.WriteLine($"Не найден клиент с кодом {clientCode}"); return false; }
            }
        ...
        public static bool deleteOrder(List<Order> orders, int key)
        {
            int orderCode;
            string input;
            Console.WriteLine("Введите код заказа для удаления (будет произведено каскадное удаление)");
            input = Console.ReadLine();
            if (input != "" && int.TryParse(input, out orderCode))
            {
                if (orders.Any(item => item.code == orderCode))
                { orders.RemoveAll(item => item.code == orderCode); }
                else { Console.WriteLine($"Не найден заказ с кодом {orderCode}"); return false; }
            }
            else { Console.WriteLine("Введённое значение не является числом, удаление отменено"); return false; }
            return true;
        }
        ...
        public static bool deleteService(List<Service> services, int key, List<Order> orders)
        {
            ...
            {
                if (services.Any(item => item.code == serviceCode))
                { services.RemoveAll(item => item.code == serviceCode); orders.RemoveAll(item => item.serviceCode == serviceCode); }
                else { Console.WriteLine($"Не найдена услуга с кодом {serviceCode}"); return false; }
            }
            else { Console.WriteLine("Введённое значение не является числом, удаление отменено"); return false; }
            return true;
        }
        ...
        public static bool deleteServiceType(List<ServiceType> serviceTypes, int key, List<Order> orders, List<Service> services)
        {
            int serviceTypeCode;
            string input;
            Console.WriteLine("Введите код типа услуги для удаления (будет произведено каскадное удаление)");
            input = Console.ReadLine();
            if (input != "" && int.TryParse(input, out serviceTypeCode))
            {
                if (serviceTypes.Any(item => item.code == serviceTypeCode))
                {
                    serviceTypes.RemoveAll(item => item.code == serviceTypeCode);
                    var serviceCodesToRemove = services.Where(item => item.typeCode == serviceTypeCode).Select(item => item.code).ToList();
                    services.RemoveAll(item => item.typeCode == serviceTypeCode);
