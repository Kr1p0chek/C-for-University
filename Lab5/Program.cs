...
internal class Program
{
    public static void Main()
    {
        ...
        while (true)
        {
            try
            {
                ...
                switch (par1)
                {
                    ...
                    case "3":
                        ...
                        {
                            case "1":
                                Console.WriteLine("Введите ключ (код), произойдёт каскадное удаление"); input = Console.ReadLine(); int id;
                                if (int.TryParse(input, out id)) ExcelWork.deleteClient(clients, id, orders); else Console.WriteLine("Ошибка, код не число"); break;
                            case "2":
                                Console.WriteLine("Введите ключ (код), произойдёт каскадное удаление"); input = Console.ReadLine();
                    ...
                    case "4":
                        ...
                        break;
                    case "5": Console.WriteLine("Ответ:" + ExcelWork.calculateTotalVladivostok(clients, serviceTypes, services, orders)); break;
                    case "6":
                        int year, month;
            ...
            {
                string input;
                Console.WriteLine("Если хотите сохранить изменения и закрыть программу введите 1"); input = Console.ReadLine();
                if (input == "1") { ExcelWork.saveDataToExcel(clients, serviceTypes, services, orders, excelFilePath); return; }
